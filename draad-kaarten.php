...
if ( ! function_exists( 'draad_maps_renderer' ) ) {
    ...
    function draad_maps_renderer ( $output, $args ) {
        ...
        $output = '<div class="draad-maps__wrapper">';
            ...
            $output .= '<search class="draad-maps__search draad-search" id="draad-maps-'. $mapId .'-search">';

                $output .= '<form class="draad-search__form">';

                    $output .= '<div class="draad-search__field">';
                    $output .= '<label class="draad-search__label" for="draad-maps-'. $mapId .'-search-input">'. __('Zoek op straat, wijk of stadsdeel', 'draad') .'</label>';
                    $output .= '<input class="draad-search__input" id="draad-maps-'. $mapId .'-search-input" type="search" placeholder="'. __('Zoeken...', 'draad') .'" list="draad-maps-'. $mapId .'-autocomplete" />';
                    $output .= '<datalist class="draad-search__autocomplete" id="draad-maps-'. $mapId .'-autocomplete"></datalist>';
                    $output .= '</div>';

                    $output .= '<button class="draad-search__submit button button--primary button--icon-only" id="draad-maps-'. $mapId .'-search-submit" role="button">
            ...
            $output .= '<div class="draad-tabs" id="tabs-'. $tabsId .'">';

                $output .= '<div class="draad-tabs__list" role="tablist" aria-labelledby="tablist-'. $tabsId .'-label">';

                    $output .= '<button class="draad-tabs__tab button button--secondary" id="tab-'. $tab1Id .'" type="button" role="tab" aria-selected="true" aria-controls="tabpanel-'. $tab1Id .'">
                                    '. draad_maps_icon( 'map' ) .'
                                    <span class="desktop-only">'. __('Kaart bekijken', 'draad') .'</span>
                                    <span class="mobile-only">'. __('Kaart', 'draad') .'</span>
                                </button>';
                    $output .= '<button class="draad-tabs__tab button button--secondary" id="tab-'. $tab2Id .'" type="button" role="tab" aria-selected="true" aria-controls="tabpanel-'. $tab2Id .'">
                ...
                $output .= '</div>';
                $output .= '<div class="draad-tabs__panel" id="tabpanel-'. $tab1Id .'" role="tabpanel" aria-labelledby="tab-'. $tab1Id .'">';

                    $output .= '<div class="draad-maps" id="draad-maps-'. $mapId .'" '. $attributes .'>';
                        $output .= '<div class="draad-maps__map" id="draad-maps-'. $mapId .'-map"></div>';
                        $output .= '<div class="draad-maps__list" id="draad-maps-'. $mapId .'-list">';
                            ...
                            if ( have_rows( 'locations', $post_id ) ) {
                                ...
                                while ( have_rows( 'locations', $post_id ) ) {
                                    ...
                                    $card .= '<h3 class="draad-infowindow__title">'. get_sub_field( 'title' ) .'</h3>';
                ...
                $output .= '</div>';
                $output .= '<div class="draad-tabs__panel" id="tabpanel-'. $tab2Id .'" role="tabpanel" aria-labelledby="tab-'. $tab2Id .'">';

                    $output .= '<div class="draad-grid">';
                        ...
                        if ( have_rows( 'locations', $post_id ) ) {
                            ...
                            while ( have_rows( 'locations', $post_id ) ) {
                                ...
                                $card .= '<h3 class="draad-card__title">'. get_sub_field( 'title' ) .'</h3>';
                                $card .= ( $content ) ? '<div class="draad-infowindow__description">'. $content .'</div>' : '';
...
if ( ! function_exists( 'draad_maps_icon' ) ) {
	...
	function draad_maps_icon( $icon ) {
		...
		if ( ! file_exists( plugin_dir_path( __FILE__ ) .'template-parts/icons/'. $icon .'.svg' ) ) {
			return '';
		}
		
		ob_start();
        include plugin_dir_path( __FILE__ ) .'template-parts/icons/'. $icon .'.svg';
        $svg = ob_get_clean();
